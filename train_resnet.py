...
if __name__ == '__main__':
    ...
    num = 0  # 加载的参数
    batch_size = 4
    net_model = "resnet50"  # "unet" "u2net_p" "unet3+" "u2net"  4次16倍下采样
    ...
    elif net_model == "resnet50":
        net = Resnet50(out_channels=len(config.classes_list)).to(device)
    ...
    for epoch in range(num + 1, Epoch):
        ...
        for i, (mask_img, real_img) in enumerate(train_load):
            ...
            # b = torch.sigmoid(F.interpolate(net(real_img), size=(mask_img.shape[2], mask_img.shape[3]), mode='nearest'))
